...
const proms = files.filter(file => file.slice(-5) === '.html').map(file => new Promise((res, rej) => fs.readFile(file, 'utf8', function(err, contents) {
    ...
    const scriptSHAs1 = !x ? [] : x.map(i => i.match(/<script.*?>([^]*?)<\/script>/));
    const styleSHAs1 = !y ? [] : y.map(i => i.match(/<style.*?>([^]*?)<\/style>/));
    
    const scriptSHAs2 = scriptSHAs1.map(i => i[0].match(/integrity="(.+?)"/)).filter(i => i && i[1]).map(i => i[1]);
    const styleSHAs2 = styleSHAs1.map(i => i[0].match(/integrity="(.+?)"/)).filter(i => i && i[1]).map(i => i[1]);
    
    const scriptSHAs = scriptSHAs1.map(getSecond).filter(hasText).map(sha256).concat(scriptSHAs2.map(i => `'${i}'`));
    const styleSHAs = styleSHAs1.map(getSecond).filter(hasText).map(sha256).concat(styleSHAs2.map(i => `'${i}'`));
